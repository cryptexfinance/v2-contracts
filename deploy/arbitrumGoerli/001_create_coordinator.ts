...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    if (process.env.COORDINATOR_ID) {
        ...
        coordinatorID = await controller.callStatic.createCoordinator({blockTag: receipt.blockNumber - 1})
        console.log(`created coordinator with ID = ${coordinatorID}. Please set this value in .env for COORDINATOR_ID`)
    }
}
